...
public class ConsoleView {
    ...
    public static void view() {
        ...
        for (int i = 1; i <= Main.GANG_SIZE - 1; i++) {
            for (int j = 1; j <= Main.GANG_SIZE; j++) {
                System.out.print(getChar(new Point(j, i)));
            }
            System.out.print("|");
        ...
        for (int j = 1; j <= Main.GANG_SIZE; j++) {
            System.out.print(getChar(new Point(j, Main.GANG_SIZE)));
        }
        System.out.print("|");
        System.out.println(PrintInfo(npcIndex));
    ...
    private static String getChar(Point position) {

        String str = "| ";
        boolean alive = false;
        for (int i = 0; i < Main.GANG_SIZE; i++) {
            if (Main.whiteSide.get(i).getPosition().isEquals(position)) {
                if (Main.whiteSide.get(i).health <= 0) // если здоровье отрицательно или равно 0
            ...
            if (Main.darkSide.get(i).getPosition().isEquals(position) && !alive) {
                if (Main.darkSide.get(i).health <= 0) // если здоровье отрицательно или равно 0
